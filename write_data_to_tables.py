...
df_movie = df.select('movies','rating', 'one_line', 'votes', 'start_year', 'end_year', 'star', 'genre', 'director')
df_movie = df_movie.join(df_title, "movies")
df_movie = df_movie.withColumn('id', row_number().over(w))

df_movie.select('id', 'title_id','rating', 'one_line', 'votes', 'start_year', 'end_year').write.insertInto('movie')
